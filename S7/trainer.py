""" Coding Session 7: fine tuning a model in TensorFlow
You can download the pre-trained checkpoint from:
...
class TFModelTrainer:
    ...
    def _optimizer(self, total_loss, global_step):
        ...
        with tf.control_dependencies(update_ops):
            ...
            if self.fine_tune: #####################################################################
                train_vars = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, "MobilenetV1/Logits")
                optimizer = optimizer.minimize(total_loss, var_list=train_vars, global_step=global_step)
            else:
    ...
    def train(self):
        ...
        with tf.Session(config=config) as sess:
            ...
            if ckpt and ckpt.model_checkpoint_path:
                restore_vars = self._variables_to_restore(ckpt.model_checkpoint_path, sess.graph)
